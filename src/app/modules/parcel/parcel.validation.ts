...
const receiverSchema = z.object({
  name: z.string().trim().min(2, 'Receiver name must be at least 2 characters').max(50, 'Name cannot exceed 50 characters'),
...
const deliveryInfoSchema = z.object({
  preferredDeliveryDate: z.coerce.date().refine(date => date > new Date(), {
...
export const trackParcelValidation = z.object({
  params: z.object({
    trackingId: z.string().trim().regex(/^TRK-\d{8}-\d{6}$/, 'Invalid tracking ID format'),
  }),
});
...
export const assignDeliveryPersonnelValidation = z.object({
  body: z.object({
    deliveryPersonnelId: z.string().regex(/^[0-9a-fA-F]{24}$/, 'Invalid user ID format'),
  }),
});
