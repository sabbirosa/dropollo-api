...
import type { IAuthResponse, IChangePassword, ILoginUser, IRegisterUser, IUpdateProfile } from './auth.interface';

const registerUser = async (userData: IRegisterUser): Promise<Omit<IUser, 'password'>> => {
...
const loginUser = async (loginData: ILoginUser): Promise<IAuthResponse> => {
  ...
  // Verify password
  const isPasswordValid = await user.isPasswordMatched(loginData.password, user.password);
...
const getMyProfile = async (userId: string): Promise<Omit<IUser, 'password'> | null> => {
